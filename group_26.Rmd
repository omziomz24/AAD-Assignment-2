...
```{r}
# Generate a binary variable named censoring that takes value of 1 if
 # medianHouseValue = 500,001; and 0 otherwise.

data$censoring <- as.integer(data$medianHouseValue==500001)
```
...
```{r}
# Maybe try a couple then decide? 
```
...
\item \textbf{Interpretability}: It can be shown with some algebra that the logistic regression is a linear regression on the log-odds. 
