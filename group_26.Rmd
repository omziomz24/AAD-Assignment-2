...
\begin{itemize}*{Addition and modification of features:}
  ...
  \item We also encoded the categorical data to numeric form so KNN can properly use it for distance calculations
  \begin{itemize}
...
\begin{itemize}*{How we used the data:}
  \item The k-NN regression model was trained using the encoded and preprocessed training data (`train_data_encoded`), which includes both numeric and one-hot encoded categorical variables.
  \item We standardized features by centering and scaling (`preProcess = c("center", "scale")`) to ensure that distance calculations are not affected by different sizes of units.
  \item We selected best `k` by performing cross validation (5-fold) over the range 1 to 15 (`tuneGrid = data.frame(k = 1:15)`).
\end{itemize}
...
Without adding any new features, KNN regression has a lower test MSE (3,697,939,911) that is about 13% lower than the GAM (test MSE = 4,257,370,057). The baseline models were tuned - KNN with 5-fold cross-validation to select the optimal k, and GAM with automatic feature selection.

After introducing additional features and encoding the categorical variables, the KNN model’s MSE dropped substantially by about 19% (to 2,999,761,722). The MSE for GAM also reduced by 20% (to 3,386,284,239). However, the KNN model still outperforms the GAM by a 11.4% lower MSE. 

KNN outperforms GAM both before and after the model improvements because its completely non-parametric nature that “makes no assumptions about the functional form” and averages responses of nearest neighbors (recall section 2.1.1 KNN Pros). This flexibility allows it to capture complex interactions introduced by new features such as cityProximityScore and the directional terms. 

On the other hand, GAM is limited by its additive structure and smoothing assumptions, which “restrict its ability to capture strong synergistic effects” unless interactions are explicitly modeled (section 2.1.1 GAM Cons). Additionally, GAM's exponential family assumption (Gaussian here) on the response variable limits its flexibility to fully model complex, censored, and non-normal housing price data, making it less suitable compared to nonparametric models like KNN.
...
The hybrid model resulted in a slightly lower test MSE (2,997,065,945) 
