...
# best k 
print(knn_reg_model1)

# KNN MSE calculation 
knn_pred1 <- predict(knn_reg_model1, newdata = test_data)
knn_MSE1 <- mean((knn_pred1 - test_data$medianHouseValue)^2) 
cat("The KNN MSE is", knn_MSE1)

```

#ALINA - knn test; bigger range for k -> k=11 not big difference
```{r}
# Basic KNN
knn_reg_model1 <- train(
  medianHouseValue ~ ., 
  ...
  method = "knn",
  preProcess = c("center", "scale"),
  tuneGrid = data.frame(k = 1:20),  # test k = 1 to 10
  trControl = trainControl(method = "cv", number = 5) 
) 
...
# best k 
print(knn_reg_model2)

# KNN MSE calculation 
knn_pred2 <- predict(knn_reg_model2, newdata = test_data_encoded)
knn_MSE2 <- mean((knn_pred2 - test_data_encoded$medianHouseValue)^2) 
cat("The KNN MSE (improved) is", knn_MSE2, "\n")
...
# train the GAM 
gam_model <- mgcv::gam(medianHouseValue ~ s(latitude) + s(longitude) + 
...
-- OK maybe not TREE cuz it usually doesnt do well with the predictions 
